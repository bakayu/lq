#!/bin/sh
#
# This script downloads and installs the latest binary release of lq.
...
main() {
  ...
  if [ -w "$INSTALL_DIR" ]; then
    mv "$TEMP_DIR/lq" "${INSTALL_DIR}/lq"
  else
    echo "Root permission is required to install lq to ${INSTALL_DIR}"
    if ! sudo mv "$TEMP_DIR/lq" "${INSTALL_DIR}/lq" 2>/dev/null; then
  ...
  chmod +x "${INSTALL_DIR}/lq"

  # Clean up the temporary directory.
  rm -rf "$TEMP_DIR"

  echo ""
  echo "lq has been installed successfully to ${INSTALL_DIR}!"
  
  # Check if the install directory is in PATH.
  case ":$PATH:" in
    *":${INSTALL_DIR}:"*) 
      echo "Run 'lq' to get started."
      ;;
    *)
      ...
      echo "  source ~/.zshrc"
      echo ""
      echo "Or run lq directly: ${INSTALL_DIR}/lq"
      ;;
  esac
}
